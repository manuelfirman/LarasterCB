...
void Snake::iniciarVariables()
{

}

void Snake::iniciarIA()
{
    // _inteligenciaArtificial->AgregarOpcion();
    //
}

void Snake::iniciarAnimaciones()
{
...
Snake::Snake(float x, float y, sf::Texture& textura, std::map<std::string, sf::SoundBuffer>& sonidos, SpawnerEnemigos& tile_spawner, Entidades& jugador) : Enemigos(tile_spawner)
{
    ...
    crearComponenteAtributos(5);                        // Atributos
...
void Snake::actualizarIA(const float DT)
{
...
void Snake::actualizarAnimacion(const float& DT)
{
...
void Snake::actualizar(const float& DT, sf::Vector2f& posMouseVista)
{
...
void Snake::renderizar(sf::RenderTarget& target, sf::Shader* sombra, const sf::Vector2f posLuz, const bool mostrar_hitbox)
{
    if (sombra)
    {
